...
public class PassiveIncome : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (_passiveIncomeData.ExtraTimePurchasedCount >= _passiveIncomeData.ExtraTimeCount)
    ...
    public void PrepareGameData(GameData gameData)
    {
        ...
        _buyButton.interactable = (gameData.Diamonds <= _passiveIncomeData.ExtraTimePrice) ? false : true;
    }

    public void GetExtraTimeForPassiveIncome()
    {       
        _currentGameData.Diamonds -= _passiveIncomeData.ExtraTimePrice;
        _diamonds.SetDiamondsScore(_currentGameData.Diamonds);
        _currentGameData.PassiveIncomeTime += _passiveIncomeData.ExtraTime;

        _passiveIncomeData.ExtraTimePurchasedCount++;
    ...
    public void ActivatePassiveIncome(int timeAfterExit)
    {
        ...
        {
            _passiveProfit = Math.Min(timeAfterExit, _currentGameData.PassiveIncomeTime) * _currentGameData.MoneyPerSec;
    ...
    public void PreparePassiveIncomeUI(int timeAfterExit, double passiveIncome)
    {
        ...
        _3xButtonReward.interactable = (_currentGameData.Diamonds <= _3xPassiveIncomePrice) ? false : true;
        ...
        TimeSpan passiveIncomeTime = TimeSpan.FromSeconds(_currentGameData.PassiveIncomeTime);
